...
const SelectContactScreen: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const renderStaticItem = ({ item }: { item: StaticAction }) => {
    const pngIcons = ['add-group', 'add-contact', 'community', 'ai-chat'];
    ...
    const getIconSource = () => {
      switch (item.icon) {
        ...
        case 'community':
          return require('../../assets/icons/communities.svg');
        case 'ai-chat':
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: ACCENT_BLUE, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
          {isPngIcon ? (
            <Image source={getIconSource()} style={{ width: 22, height: 22 }} />
          ) : (
            <SvgIcon 
              name={item.icon as any} 
              size={22} 
              color={item.icon === 'communities' ? '#000000' : '#ffffff'} 
            />
          )}
        </View>
        <Text style={{ color: '#ffffff', fontSize: 16, flex: 1 }}>{item.label}</Text>
        {item.rightAccessory}
      </TouchableOpacity>
    );
  };
    ...
    return (
      <TouchableOpacity onPress={() => openChat(item)} activeOpacity={0.7} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 10 }}>
        {item.imageAvailable && item.image ? (
          ...
          <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: '#1f2c34', alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
            <Text style={{ color: '#cfd8dc', fontWeight: '700' }}>{initials || 'U'}</Text>
          </View>
        )}
        <View style={{ flex: 1 }}>
          <Text style={{ color: '#ffffff', fontSize: 16 }} numberOfLines={1}>{item.name || phone || 'Unknown'}</Text>
          <Text style={{ color: '#9aa0a6', fontSize: 12 }} numberOfLines={1}>{phone || 'Hey there! I am using TalkWave'}</Text>
        </View>
      </TouchableOpacity>
    );
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0b141a' }}>
      ...
      <View style={{ 
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', minHeight: 44, paddingBottom: 10 }}>
          <TouchableOpacity onPress={onBack} activeOpacity={0.7} style={{ padding: 8, marginRight: 6 }}>
            <SvgIcon name="arrow-left" size={20} color="#ffffff" />
          </TouchableOpacity>
          <View style={{ flex: 1 }}>
            <Text style={{ color: '#ffffff', fontSize: 18, fontWeight: '700' }}>Select contact</Text>
            <Text style={{ color: '#9aa0a6', fontSize: 12 }}>{totalContacts} contacts</Text>
          </View>
          <TouchableOpacity onPress={() => setMenuOpen(true)} activeOpacity={0.7} style={{ padding: 8 }}>
            <SvgIcon name={'dots'} size={22} color={'#cfd8dc'} />
          </TouchableOpacity>
        </View>
        {/* Search */}
        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#111111', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8, marginTop: 8 }}>
          <SvgIcon name={'search'} size={18} color={'#9aa0a6'} />
          <TextInput
            value={query}
            onChangeText={setQuery}
            placeholder={'Search name or number'}
            placeholderTextColor={'#9aa0a6'}
            style={{ flex: 1, color: '#ffffff', paddingLeft: 8 }}
          />
        </View>
      </View>
        ...
        ListHeaderComponent={
          <View>
            <FlatList
              ...
              ItemSeparatorComponent={() => <View style={{ height: 1, backgroundColor: '#1f2c34' }} />}
            />
            <View style={{ height: 12 }} />
            <Text style={{ color: '#9aa0a6', fontSize: 12, marginVertical: 6 }}>Contacts on TalkWave</Text>
          </View>
        }
      ...
      <Modal visible={menuOpen} transparent animationType="fade" onRequestClose={() => setMenuOpen(false)}>
        <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', alignItems: 'flex-end' }} activeOpacity={1} onPress={() => setMenuOpen(false)}>
          <View style={{ marginTop: 60, marginRight: 12, backgroundColor: '#1f2c34', borderRadius: 12, paddingHorizontal: 16, paddingVertical: 8, width: 220 }}>
